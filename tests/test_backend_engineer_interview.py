...
from backend_engineer_interview import app
...
class TestPostApplication:
    ...
    def test_search_application_by_id_endpoint(self: Self, test_client: TestClient) -> None:
        employee_id = 1
        ...
        response = test_client.get(f"/v1/application?employee_id={employee_id}")
        assert response.status_code == 200
